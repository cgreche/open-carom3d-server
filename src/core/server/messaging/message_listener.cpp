...
void MessageListener::EventHandler::onClientDataReceived(nettools::ntClient* ntClient, unsigned char *data, unsigned int dataLen) {
    ClientSession *client = (ClientSession*)ntClient->bindedData();
    ParsedDataResultInfo parsedData = m_messageListener.m_messageParser.parseMessageData(client->inDataCryptoCtx(), data, dataLen);
...
void MessageListener::EventHandler::onClientDataSent(nettools::ntClient* ntClient, const unsigned char *data, unsigned int dataLen) {
    //TODO
}
